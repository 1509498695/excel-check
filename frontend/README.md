...
- 个人校验步骤 3 的规则区包含 `手动规则编排 / 智能添加规则` 两个页签。智能页签现在保留「目标变量」多选、规则描述输入框和“允许 AI 自动补齐数据源/变量”开关：默认关闭时用户先从变量池选择一个或多个变量，再用短模板描述规则，AI 只基于所选变量生成草稿；开启后目标变量可不选，开关下方会出现 `数据源 / sheet分页 / 变量选择` 固定输入区，规则描述框仍只填写规则语义。默认模板为 `筛选：- 字段=值 / - 字段唯一`、`Key值选择：字段名`、`判定：字段不能为空 / 字段不能重复 / 字段必须重复 / 字段=值 / 字段等于字段X`，用户不需要理解 `规则类型 / 目标字段 / Key字段 / 规则参数` 等技术槽位。开启自动补齐时，`数据源 / sheet分页 / 变量选择 / 判定` 会作为 AI 校验前置门禁，缺失项会在输入区直接提示；`变量选择` 输入框会保留用户输入过程中的英文逗号，真正提交时再规范化字段列表。后端会把这些线索作为 `workflow_hints` 优先用于补齐数据源 / 变量，并兼容旧自然句模板、旧 v3 技术模板、旧三段模板和自由文本。AI 校验前还会做规则候选批判：按当前 10 类规则给候选打分并收窄类型，发现一条规则混入多个最终断言时提示拆分，发现聚合、平均值、公式等不支持能力时直接展示不可添加原因。输入框上方提供两类辅助入口：基于已选变量 `single/composite`、字段、组合列和 Key 自动生成的推荐规则，以及默认收起、可按需展开的系统内置规则模板 / 常用案例。点击模板或推荐卡片会按短模板格式预填描述和解析线索，仍需用户手动点击 AI 校验、执行预校验并确认添加；“载入案例”同样优先使用当前已选变量对应的推荐短模板，没有推荐项时回填短模板 fallback。“优化输入”会把用户文本规范化为规则 DSL，例如 `composite_condition_check` 下的 `筛选：- FIELD != VALUE / - FIELD 唯一`、`Key：FIELD`、`断言：A 必须重复`，并尽量保留字段、筛选、Key、引用对象、正则、排序等解析线索；聚合、平均值等当前规则库不支持的描述会提示用户确认，而不会硬套成现有规则。AI 校验结果会以解释卡展示 `ready / needs_input / rejected`：说明为什么匹配到某个规则类型、还缺什么、点击哪里修复，并把缺口归类为缺数据源、缺 Sheet、缺字段、需要 Key 字段、缺规则参数或规则类型不支持。ready 草稿的预校验结果会继续聚合异常原因、规则名、定位样例和修复建议；发现异常或数据源失败时，可点击“带调整建议重新生成”把这些反馈作为临时 extra_hints 重新生成草稿。结果为 `needs_input` 且自动补齐开启时，可点击“一键补齐并添加”重新生成草稿：后端会对未保存的数据源临时读取表头，Key 只使用用户明确输入或 `INT_ID / INT_Id / ID`，预校验通过后一次性保存数据源、组合变量和规则，不会立即执行正式校验。信息仍不足时结果区会提示补充完整描述，或关闭自动补齐并选择已有变量池变量。后端覆盖当前 10 类已有规则，草稿会先用临时 TaskTree 在面板内执行预校验，不会直接改配置；`ready` 且预校验成功后才可添加并立即保存，`rejected` 会展示当前规则库缺失能力和扩展建议。
- 智能添加规则的前端线索抽取与后端兜底抽取保持同一语义：筛选支持 `= / != / > / < / 包含 / 不包含 / 非空`，组合断言支持 `不能为空 / 正则 / 唯一 / 必须重复 / 等于 / 不等于 / 大于 / 小于`，跨组比较支持 `eq / ne / gt / lt / not_null` 和“基准为准 / 双向检查”。多组串行 / 映射规则只在用户按清晰节点 DSL 描述时抽取，无法确定时返回 `needs_input`。
- 对短模板里的 `筛选：FIELD=A,B + Key值选择 + 多字段两组相等`，优化输入会优先规范为 `dual_composite_compare` DSL：拆出左侧筛选、右侧筛选、Key 和比较字段，避免误判为固定值比较。
