...
- `src/App.vue` 顶部导航里的“管理后台”入口现在对项目管理员和超级管理员显示。
- `src/views/AdminView.vue` 当前已支持：
  - 项目列表首屏默认选中第一个可管理项目
  - 成员列表展示 `归属项目`
  - 对普通用户执行“调整归属项目”
  - 项目管理员仅查看并管理自己可管理的项目
  - 项目管理员可编辑自己管理项目的名称/描述，但不能创建或删除项目
- `src/api/admin.ts` 新增普通用户归属项目调整接口封装。
- 前端继续信任后端返回的 `current_project_id / current_role`，登录默认项目不再依赖任何前端本地猜测。
- 本轮回归：
  - `npm run build`：通过
  - 运行态补充验证：`http://127.0.0.1:5174` 可访问
...
- **API 模块**：`src/api/auth.ts`（注册、登录、当前用户等）、`src/api/admin.ts`（项目创建、修改、删除与成员管理接口）。
...
- **`App.vue`**：头部增加用户下拉菜单（个人资料、退出等）；**项目管理员 / 超级管理员** 可见 **管理后台** 导航入口。
- **管理后台项目交互**：`AdminView.vue` 现已支持：
  - 超级管理员：创建项目、编辑项目、删除普通项目、管理成员
  - 项目管理员：查看自己可管理项目、编辑项目名称/描述、管理普通用户成员
  - 成员区展示归属项目，并支持调整普通用户归属项目
  - 页面打开时默认选中第一个可管理项目，避免成员区空白
- **管理后台成员删除规则**：
  - 默认项目中删除成员 = 删除用户账号
  - 其他项目中删除成员 = 迁移到默认项目
  - 成员删除统一需要二次确认
- **删除项目空响应兼容**：管理后台删除项目接口仍返回 `204 No Content`，`apiFetch.ts` 已兼容空响应体，不再出现 `Unexpected end of JSON input`。
...
## 2026-04-17 默认管理员登录修复说明

- 当前前端登录页 `/login` 不再需要规避后端 `500`；问题已经从后端启动初始化链路修复。
- 系统启动时会自动：
  - 初始化运行时数据库表结构
  - 确保默认项目 `默认项目` 存在
  - 确保默认超级管理员 `admin / 123456` 可登录
- 当前真实联调结果：
  - `GET http://127.0.0.1:8000/health`：`200`
  - `GET http://127.0.0.1:5173/login`：`200`
  - `POST /api/v1/auth/login` 使用 `admin / 123456`：成功返回 token
