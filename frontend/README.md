...
- 步骤 1 头部的 `数据源路径管理` 会把路径替换拆成 `本地路径 / SVN 路径` 两组；本地组管理本地 Excel / CSV 与 SVN 工作副本路径，SVN 组管理远端 HTTP URL，两组都支持保存目录列表、一键替换文件名前目录，以及对已保存目录做行内新增、编辑和删除。
- 两组路径替换都会先做整批预校验：数据源元数据和受影响变量预览只要有一项失败，就整批回滚，不会把坏路径保存进配置。
