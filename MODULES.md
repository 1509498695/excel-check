...
依据 [README.md](README.md)、[需求文档.md](需求文档.md) 第 2 章架构与目录、[frontend/README.md](frontend/README.md) 及后端路由 [backend/app/api/router.py](backend/app/api/router.py)。
...
| [需求文档.md](需求文档.md) | SDD：与代码对齐的架构、页面设计、协议与边界。 |
| [frontend/README.md](frontend/README.md) | 前端子项目说明与模块推进建议。 |
| [PROJECT_RECORD.md](PROJECT_RECORD.md)、[CHANGELOG.md](CHANGELOG.md) | 进度与版本记录（README「相关文档」引用）。 |
| [CODEX_DEVELOPMENT_WORKFLOW.md](CODEX_DEVELOPMENT_WORKFLOW.md) | 开发流程约定（若团队使用 Codex/Agent 工作流）。 |
...
**说明**：主工作台步骤 3 仅静态规则；主工作台静态规则仍主要面向**单变量**（组合变量进入 `TaskTree` 但步骤 3 会过滤）。固定规则页在 `version = 4` 下可绑定单变量与组合变量规则（含 `composite_condition_check`），与主工作台数据源/变量池持久化隔离。以上与 [需求文档.md](需求文档.md) 第 1.3 节「能力边界」及 [README.md](README.md)「当前关键能力」一致。
