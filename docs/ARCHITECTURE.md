...
- 远程浏览器可通过 `/api/v1/sources/upload` 上传 `.xlsx/.xls/.csv`，文件按 `<runtime_uploads>/<project_id>/<user_id>/` 隔离保存，再复用现有 `DataSource.pathOrUrl` 与统一执行引擎。
...
- 上传目录 `backend/.runtime_uploads/` 当前不做自动清理、不做病毒扫描；管理员需要按磁盘空间定期维护。
