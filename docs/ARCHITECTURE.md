...
- 区分两种使用场景：
  - **临时编排**（主工作台 `/`）：项目配置不持久化到固定规则配置，刷新即丢；适合一次性排查。
  - **长期复用**（固定规则页 `/fixed-rules`）：按 `project_id` 持久化到 `version=4` 配置，可反复执行，可接 SVN 拉新。
- 多用户多项目隔离：固定规则配置按 `project_id` 隔离，工作台配置按 `project_id + user_id` 隔离。
...
- 主工作台四步工作流：数据源 → 变量池 → 规则编排 → 结果。
- 固定规则页独立 `version=4` 配置：`sources / variables / groups / rules` 一体化保存。
...
- `svn` 作为主工作台独立 source 类型：未开放（固定规则页通过 `svn-update` 间接使用 SVN）。
- `regex` 规则：未注册。
...
### 5.4 主工作台 `/engine` `/workbench`

| 方法 | 路径 | 说明 |
|:---|:---|:---|
| `POST` | `/engine/execute` | 入参 `TaskTree`，出参统一响应协议。 |
| `GET` | `/workbench/config` | 读取当前用户在当前项目下的工作台配置。 |
| `PUT` | `/workbench/config` | 保存工作台配置（前端 2 秒防抖自动调用）。 |

### 5.5 固定规则 `/fixed-rules`

| 方法 | 路径 | 说明 |
|:---|:---|:---|
| `GET` | `/fixed-rules/config` | 读取当前项目的固定规则配置；可能附带 `meta.config_issues`。 |
...
### 6.2 固定规则页 `/fixed-rules`

[FixedRulesBoard.vue](../frontend/src/views/FixedRulesBoard.vue)：与主工作台同套 4 步骨架（顶部 Stepper + KPI + 01–04 模块）；规则区复用 `WorkbenchRuleOrchestrationPanel` 同款样式但走自己的 `useFixedRulesStore`；结果区直接复用 `ResultBoardPanel`。
...
| 资源 | 隔离维度 |
|:---|:---|
| 固定规则配置 `FixedRulesConfigRecord` | `project_id` |
| 工作台配置 `WorkbenchConfigRecord` | `project_id + user_id` |
