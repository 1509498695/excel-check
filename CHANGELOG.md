...
- 修复个人校验 `/` 与项目校验 `/fixed-rules` 的 `组合分支校验` 非空断言：组合变量执行行集会保留空 `key_column` 行，分支 `not_null` 可报出 Excel 空单元格、`NaN`、空字符串和纯空白字符串；跨组变量等依赖 Key 对齐的规则仍按历史行为忽略空内部 Key 行。
- 个人校验 `/` 与项目校验 `/fixed-rules` 的 `跨组变量校验` 录入入口调整：顶部不再单独展示“目标变量”，基准变量改到组合条件段内下拉选择，保存协议保持不变。
- 个人校验 `/` 与项目校验 `/fixed-rules` 的 `跨组变量校验` 新增左右“关联 Key 字段”：旧规则默认按 `__key__` 兼容，用户可显式选择 `INT_Level` 等业务字段，使同一组合变量左右筛选后即使开启 `Key 后追加序号` 也能按业务 Key 对齐。
- 个人校验 `/` 与项目校验 `/fixed-rules` 的 `跨组变量校验` 新增左右筛选：支持同一组合变量拆成两个筛选子集后按 Key 对比，也支持两个不同组合变量各自筛选后对比；旧配置默认空筛选保持兼容。
- CSV 数据源接入已移除：前端新增数据源不再展示 CSV，后端上传、本地选择、能力声明与执行链路均不再接受 CSV；历史 CSV 配置会提示删除后改用 Excel 或 SVN Excel。
- 个人校验 `/` 与项目校验 `/fixed-rules` 的新增数据源弹窗默认选中 `SVN（推荐 HTTP 链接）`，便于优先使用内网共享部署下的远端 SVN 文件接入。
- 个人校验 `/` 新增 `SVN 更新`：点击后先保存当前个人配置，再刷新该用户个人配置中的本地 SVN 工作副本或远端 SVN 缓存，结果区展示更新汇总与逐项输出。
- 个人校验 `/` 与项目校验 `/fixed-rules` 的规则配置新增可选 `结果显示字段`：字段范围限定为当前规则关联变量内字段，异常结果表与 Excel 导出会展示该字段值，未配置时显示字段列为空。
- 个人校验 `/` 与项目校验 `/fixed-rules` 的 `多组映射校验` 筛选失败排除范围新增 `判定值`：只有 Excel 行号命中范围且当前筛选字段值命中逗号分隔的判定值集合时，才会从异常结果中移除。
...
- 数据源类型入口收口：飞书仍未完整接入，新增数据源下拉中显示为“占位”并禁用选择，历史配置仍可展示和删除。
...
- 浏览器上传 Excel 的默认落盘目录调整为 `backend/.runtime_uploads/local_excel/<project_id>/<user_id>/`，继续保留项目与用户隔离。
- 个人校验 `/` 与项目校验 `/fixed-rules` 的 `数据源路径管理` 收口为仅管理远端 SVN 目录 URL；本地 Excel 不再提供本地路径替换管理，历史本地路径配置字段继续保留兼容。
- 本机共享部署升级：新增 `scripts/start-local-deploy.ps1`，支持前端构建后由 FastAPI 单服务托管；后端监听、端口、前端产物目录、CORS、上传大小、JWT 密钥与默认管理员密码均可通过环境变量配置。
- 数据源新增浏览器上传链路：`POST /api/v1/sources/upload` 支持 `.xlsx/.xls`，按当前项目与用户隔离保存到 `backend/.runtime_uploads/local_excel/`，远程访问用户无需依赖服务器本机 `tkinter` 文件选择框。
...
- 修复步骤 2 变量弹窗的前端错误拦截：SVN Excel 数据源不再被误判为“仅本地 Excel 可提取字段”，个人校验与项目校验现在都可直接基于 SVN `.xls/.xlsx` 数据源添加单变量和组合变量。
