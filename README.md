...
- 当前环境仍未检测到 `svn` CLI，因此固定规则页点击 `SVN 更新` 时会返回明确提示，不影响本地 Excel 固定规则主流程。
...
- 当前环境未检测到 `svn` CLI，因此 `SVN 更新` 会返回明确提示，不影响本地 Excel 固定规则主流程。
...
- 固定规则反向压测样例：
  - `D:\projact_samo\GameDatas\datas_qa88\items.xls -> items -> INT_ID > 10000`
  - 返回 `Execution Completed / total_rows_scanned = 3917 / failed_sources = [] / abnormal_results = 770`
- `SVN 更新` 在 `svn_enabled = true` 且当前环境缺少 CLI 时会返回：
...
- 固定规则模块已经独立落地：
  - 支持规则级文件绑定：每条规则单独维护 `file_path / sheet / column`
  - 支持规则组新增、改名、删除与规则自动迁移到 `未分组`
  - 支持页面自动读取已保存配置并默认执行全部规则组
  - 支持固定显示的 `SVN 更新` 按钮，统一更新当前所有已配置路径
